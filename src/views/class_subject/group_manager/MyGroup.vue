<template>
  <div id="my-group" class="grid wide">
    <div class="row">
      <div
        class="col l-8 l-o-2 c-10 c-o-1"
        v-if="
          Object.keys($store.state.CLASSSUBJECTDETAILS.group.myGroup).length ==
            0
        "
      >
        Chưa có nhóm
      </div>
      <div class="col l-8 l-o-2 c-10 c-o-1" v-else>
        <div
          v-if="$store.state.CLASSSUBJECTDETAILS.group.isGroupLeader == true"
        ...
        <h2 style="margin-bottom: 5px;">
          Tên nhóm: {{ $store.state.CLASSSUBJECTDETAILS.group.myGroup.name }}
        </h2>
        <h3 style="margin-bottom: 15px;">
          Mô tả:
          {{ $store.state.CLASSSUBJECTDETAILS.group.myGroup.description }}
        </h3>
        <div class="group-note">
          <div class="note-title">Ghi chú:</div>
          <div class="note-content">
            {{ $store.state.CLASSSUBJECTDETAILS.group.myGroup.note }}
          </div>
        </div>
        ...
        <div class="post-notify-box">
          <div class="post-notify-full">
            <txtarea-editor
              ...
              content="Nội dung thông báo cho nhóm"
        ...
        <div class="list-notify-box">
          <div class="list-notify">
            <div
              ...
              v-for="(notify, index) in $store.state.CLASSSUBJECTDETAILS.group
                .myGroup.notifies"
              :key="notify.id"
            >
    ...
    <!--  FORM EDIT GROUP INFO  -->
    <transition name="fade">
      <div
        class="form-box form-edit-2"
        v-if="formEditGroupInfo"
        id="form-edit-group-info"
      >
        <div class="overlay" @click="formEditGroupInfo = false"></div>
        <form @submit.prevent="submitEditGroupInfo" class="form-body">
          <div class="form-block">
            <h3 class="form-title">Cập nhật thông tin nhóm</h3>
            <div class="form-group">
              <input
                type="text"
                :class="{ valid: dataEditGroupInfo.name }"
                v-model="dataEditGroupInfo.name"
              />
              <label>
                Tên nhóm
              </label>
            ...
            <div class="form-group">
              <input
                type="text"
                :class="{ valid: dataEditGroupInfo.description }"
                v-model="dataEditGroupInfo.description"
              />
              <label>
                Mô tả
              </label>
            ...
            <div class="form-group">
              <span
                class="input"
                :class="{ valid: dataEditGroupInfo.note }"
                role="textbox"
                contenteditable="plaintext-only"
                @input="editGroupNoteInput($event)"
                >{{ getNoteMyGroup }}</span
              >
              <label>
                Ghi chú
            ...
            <div class="form-btn-group">
              <button
                class="btn btn-danger"
                @click.prevent="formEditGroupInfo = false"
              >
                Hủy
              </button>
              <button
...
export default {
  ...
  methods: {
    ...
    clickEditGroupInfo() {
      this.formEditGroupInfo = true;
      let myGroup = this.$store.getters.getMyGroup;
      this.dataEditGroupInfo["name"] = myGroup.name;
      this.dataEditGroupInfo["description"] = myGroup.description;
      this.dataEditGroupInfo["note"] = myGroup.note;
    },
    editGroupNoteInput(event) {
      this.dataEditGroupInfo.note = event.target.innerText;
    },
    submitEditGroupInfo(event) {
      let token = localStorage.getItem("token_user");
      if (token) {
        if (!this.dataEditGroupInfo.name) {
        ...
        console.log(form);
        form.querySelector(".btn.btn-success").classList.add("loading");
        form.querySelector(".btn.btn-success").classList.add("disabled");
        apiGroup
          .update(
            token,
            this.roomId,
            this.$store.getters.getMyGroup.id,
            this.dataEditGroupInfo
          )
          .then(res => {
            if (res.data.status) {
              this.$store.commit("updateMyGroup", res.data.data);
              this.$customjs.showToast({
                title: "Thành công",
                message: res.data.message
              });
              this.formEditGroupInfo = false;
            } else {
              this.$customjs.showToast({
                title: "Thất bại",
                message: res.data.message,
          ...
          .then(() => {
            form.querySelector(".btn.btn-success").classList.remove("loading");
            form.querySelector(".btn.btn-success").classList.remove("disabled");
          });
      }
    },
          ...
          apiGroup
            ...
            .then(res => {
              if (res.data.status) {
                ...
                this.$store.commit("createGroupNotify", dataCreate);
                // reset value
        ...
        if (token) {
          ...
          apiGroup
            .updateNotify(
              ...
              this.roomId,
              this.$store.getters.getMyGroup.id,
              this.dataEditNotify.data.id,
              formData
            )
            .then(res => {
              if (res.data.status) {
                this.$store.commit("updateGroupNotify", {
    ...
    deleteComment(replyId, notifyId, notifyIndex, cmtIndex) {
      let token = localStorage.getItem("token_user");
      if (token) {
        this.$confirm("Xác nhận xóa nhận xét?", "Cảnh báo", {
          ...
          .then(() => {
            apiGroup
              ...
              .then(res => {
                if (res.data.status) {
                  this.$store.commit("deleteGroupNotifyReply", {
                    notifyIndex,
                    cmtIndex
                  });
                  this.$customjs.showToast({
      ...
      if (token) {
        ...
        apiGroup
          .updateReply(
            ...
            this.roomId,
            this.$store.getters.getMyGroup.id,
            this.dataEditCmt.group_notify_id,
            this.dataEditCmt.id,
          ...
          .then(res => {
            if (res.data.status) {
              ...
              this.$store.commit("updateGroupNotifyReply", {
    ...
    deleteGroup() {
      let token = localStorage.getItem("token_user");
      if (token) {
        this.$confirm("Xác nhận giải tán nhóm?", "Cảnh báo", {
          confirmButtonText: "Xác nhận",
          cancelButtonText: "Hủy",
          type: "warning"
        })
          .then(() => {
            apiGroup
              .delete(token, this.roomId, this.$store.getters.getMyGroup.id)
              .then(res => {
                if (res.data.status) {
  ...
  computed: {
    getNoteMyGroup() {
      return this.$store.state.CLASSSUBJECTDETAILS.group.myGroup.note;
    }
  },
  components: {
    txtareaEditor
